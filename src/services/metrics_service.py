...
class MetricsRepository:
    ...
    def add_new_user(self, user_id: int, date_str: str, demographics: Optional[Dict[str, Any]] = None) -> None:
        uid = str(user_id)
        # Create/update user record
        self.users_col.document(uid).set(
            {
        ...
        self.daily_col.document(date_str).set(
            {
    ...
    def add_active(self, user_id: int, date_str: str) -> None:
        uid = str(user_id)
        self.daily_col.document(date_str).set(
            {"active_users": DB.array_union([uid])}, merge=True
        )

    def inc_feature(self, date_str: str, feature_key: str, by: int = 1) -> None:
    ...
    def update_demographics(self, user_id: int, demographics: Dict[str, Any]) -> None:
        uid = str(user_id)
        clean = {k: v for k, v in demographics.items() if v is not None}
        if not clean:
            return
        self.users_col.document(uid).set({"demographics": clean}, merge=True)

    # Reads
    def get(self) -> Dict[str, Any]:
        users: Dict[str, Any] = {}
        for doc in self.users_col.stream():
            users[doc.id] = doc.to_dict() or {}

        daily: Dict[str, Any] = {}
        for day_doc in self.daily_col.stream():
            rec = day_doc.to_dict() or {}
            # Load feature usage subcollection
            feats: Dict[str, int] = {}
            for fdoc in self.daily_col.document(day_doc.id).collection("features").stream():
                d = fdoc.to_dict() or {}
...
class MetricsService:
    ...
    def record_start(self, user_id: int, language_code: Optional[str] = None, username: Optional[str] = None, first_name: Optional[str] = None, last_name: Optional[str] = None) -> None:
        ...
        self._repo.add_new_user(user_id=user_id, date_str=today, demographics={k: v for k, v in demographics.items() if v})
        self._repo.inc_feature(today, "command:/start")

    def record_interaction(self, user_id: int, feature_key: str) -> None:
        today = self._today_str()
        self._repo.add_active(user_id=user_id, date_str=today)
        self._repo.inc_feature(today, feature_key)

    def record_demographics(self, user_id: int, demographics: Dict[str, Any]) -> None:
        self._repo.update_demographics(user_id, demographics)
    ...
    def feature_usage(self, days: int = 7, top_n: int = 10) -> List[Tuple[str, int]]:
        data = self._repo.get()
        daily: Dict[str, Any] = data.get("daily", {})
        today = datetime.now(timezone.utc).date()
        want = {(today - timedelta(days=i)).isoformat() for i in range(days)}
    ...
    def retention_next_day(self, days: int = 14) -> List[Tuple[str, float]]:
    ...
    def today_overview(self) -> Dict[str, Any]:
        today = self._today_str()
        data = self._repo.get()
